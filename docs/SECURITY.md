...
| SEC-015 | The container scan referenced an invalid mutable Trivy action tag in workflows with security-event and release permissions. Third-party tag compromise can execute attacker code inside CI. | High / 8.8 (`AV:N/AC:L/PR:N/UI:R/S:U/C:H/I:H/A:H`) | A compromised upstream tag runs when a maintainer pushes a release, reads the workflow token, requests OIDC credentials, or tampers with the published image. | Updated to signed Trivy Action v0.36.0 and pinned its immutable commit `ed142fd0673e97e23eac54620cfb913e5ce36c25` in both CI and release workflows. Least-privilege job separation limits non-release exposure. |
...
| R-006 | Third-party CI actions and container base images remain supply-chain dependencies. Trivy is commit-pinned, while several GitHub/Docker actions use maintained major tags for Dependabot compatibility. | High | Progressively pin every high-privilege action and image by immutable digest, verify signatures, generate provenance, scan on promotion, and enforce admission policy. Keep Dependabot enabled so pinned revisions receive update proposals. |
